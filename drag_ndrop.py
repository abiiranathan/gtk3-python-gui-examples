...
class DragDropWindow(Gtk.Window):
    def __init__(self):
        ...
        image_button = Gtk.RadioButton.new_with_label_from_widget(
            None, "Images")
        image_button.connect("toggled", self.add_image_targets)
        button_box.pack_start(image_button, True, False, 0)

        text_button = Gtk.RadioButton.new_with_label_from_widget(
            image_button, "Text")
...
class DropArea(Gtk.Label):
    ...
    def on_drag_data_received(self, widget, drag_context, x, y, data, info, time):
        if info == TARGET_ENTRY_TEXT:
            ...
            print("Received pixbuf with width %spx and height %spx" %
                  (width, height))
